...
6. 总库路由文档登记：
   yuque_create_doc(总库, 标题="{domain}") → body 写入 index_books + source_books
...
```
(跳过 1，直接用已有子库 book_id)
2-5. 同新建域（列文档 → 评分 → 聚合 → 写入）

6. 更新总库路由文档：
   yuque_update_doc → 更新 source_books 中对应条目的 last_built
```
...
2. 遍历每个 source_book：
   ...
   b. 对每个变更文档：
      ...
      - 逐关键词写入子索引库：
        ...
        · 关键词索引不存在 → yuque_index_create 新建
   c. 更新该 source_book 条目的 last_built → 写回总库路由文档
```
