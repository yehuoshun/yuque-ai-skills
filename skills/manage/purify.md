...
## 本批文档
{每篇：clean_title + 关键词列表（从正文提取 3-8 个技术关键词）+ 前 200 字摘要}

## 分类规则（严格按优先级执行）

### 1. 关键词提取
从每篇文档的 clean_title + 摘要中提取 3-8 个技术关键词。
关键词 = 技术术语/框架名/概念名，不要动词/形容词/通用词。
例：「Spring Boot 自动配置原理」→ ["SpringBoot", "自动配置", "AutoConfiguration", "EnableAutoConfiguration"]
...
1. 孤类清理
   遍历所有类，doc_count < 2 → 强制合并到关键词重叠最多的类

2. 大类别拆分
   遍历所有类，doc_count > 20 → 提取该类下所有文档的二级关键词
   → 按二级关键词聚类，拆成 2-3 个子类
   → 子类命名遵循命名规范
...
3. 语义重复合并
   两两比较所有同级类名 + 关键词列表：
   - 类名相同 → 合并
   - 关键词重叠 ≥ 50% → LLM 判断是否同义（如「容器」vs「Docker」→ 合并为「Docker」）
   - 关键词重叠 < 50% → 保留
