...
**优点**：零预处理、零索引维护、零向量库。标题质量好的知识库即开即用。

**标题泛化不够时**：LLM 可先用 `yuque_search` 在全库范围搜索，再用 `yuque_get_doc` 读具体内容。`yuque_batch_get_docs_body` 可批量并发获取多篇正文。

### 搜索流程

1. **TOC 导航**（首选）：`yuque_list_repos` → 选知识库 → `yuque_list_toc` → 按标题匹配选文档 → `yuque_get_doc`
2. **全库搜索**（降级）：`yuque_search` 搜关键词 → 从结果选文档 → `yuque_get_doc`
3. **批量补读**：需要多篇时用 `yuque_batch_get_docs_body` 并发获取
