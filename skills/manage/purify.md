...
```
对每篇需要清洗的文档：
  yuque_get_doc → 获取 body
  LLM 清洗 → { clean_title, clean_body, changes }
  记录清洗结果
...
LLM 分析所有清洗后文档，输出分类方案：

```
输入：{n} 篇文档的 {clean_title + 内容摘要（前200字）}
  ...
  "assignments": [
    {
      ...
      "secondary_categories": [["一级", "三级"]]  // 可选，多目录
    }
  ]
}
```
