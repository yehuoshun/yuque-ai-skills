...
| 层 | 内容 | 作用 |
|----|------|------|
| 总库路由文档 | JSON `{index_books, source_books}` | 搜索路由 + 构建追踪（源库映射、last_built 增量基准） |
...
正文 — 两段式 JSON，路由指针 + 源库映射表：

```json
{
  "index_books": [
    {"did": 271898913, "ns": "yehuoshun/index-java-1"}
  ],
  "source_books": [
...
| 字段 | 用途 |
|------|------|
| `index_books` | 搜索路由：指向子索引库的索引文档，通过 did/ns 直接 GET |
| `source_books` | 构建追踪：该域覆盖的源知识库数组，`last_built` 为增量构建的时间基准。新增源库追加条目，`last_built` 为空表示未构建过 |

> 一个关键词可能对应多个索引文档（如超过 195KB 分片或不同子库），`index_books` 是数组。
...
6. yuque_create_doc → 总库创建路由文档，写入 index_books + source_books（last_built 为当前时间）
```
