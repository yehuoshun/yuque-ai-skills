...
## 已有分类树
{当前分类树 JSON，含每个类的文档数。第一批时为空对象 {}，LLM 从零构建}
...
for 每批 10 篇：
  ...
  body_cache[doc_id] → LLM 清洗 → { clean_title, clean_body, keywords, changes }
...
```
目标库：
  先 yuque_list_repos 检查是否已存在「{源库名}-净化版」
  不存在 → yuque_create_repo(name="{源库名}-净化版", slug)
    slug 生成规则：{源库slug}-purified-{时间戳秒}（如 java-notes-purified-1714473600）
  已存在 → 告用户「目标库已存在，将追加写入」，等确认
...
for 每篇文档（按分类分组，同分类内串行，不同分类并发 3）：
  ...
  // 2 层：toc_cache["Java"]["children"]["集合"]["uuid"]
  // 3 层：toc_cache["Java"]["children"]["集合"]["children"]["List"]["uuid"]
  primary_uuid = toc_cache[一级][children][二级][uuid]  // 或继续 .children.三级.uuid
  ...
  if secondary_categories:
    for each sec_cat in secondary_categories:
      ...
      sec_uuids.append(sec_uuid)
    // 收集完所有副分类 UUID 后，一次调用批量复制
    yuque_clone_doc_to_toc(
      book_id=目标库id,
      doc_id=doc_id,
      target_uuids=sec_uuids
    )
