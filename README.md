...
| `yuque_index_create` | 创建细粒度关键词索引文档：一个关键词一篇索引文档，标题为精确知识点名称。⚠️ 每个关键词只对应 1 篇源文档。LLM 先做关键词质量过滤（丢弃栏目名/编号/标签等无效词），再写 body（关键词搜索面 + 摘要 + entry 指针，did/ns/t/s/url/w 全部必填，w 为 1-10 权重）。传 route_book_id 后自动同步总库路由 |
