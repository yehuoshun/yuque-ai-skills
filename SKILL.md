...
正文 — source_books 数组，记录该关键词下覆盖的源知识库：

```json
[
  {"book_id": 70910909, "namespace": "yehuoshun/dil9w3"},
...
> **关键词级路由**：一篇总库路由文档对应一个关键词。标题 = 关键词，body = source_books 数组。搜索时先搜总库，客户端按标题精确匹配（`token.toLowerCase() === title.toLowerCase()`）确认关键词已索引 → 同样方式搜子库命中索引文档 → 展开 entries。
>
> ⚠️ 语雀 v2 API 的 `in:title` 语法与 `scope` 参数不兼容（传 scope 后 `in:title` 返回 0），因此改用客户端标题过滤。
>
> 不再需要 `index_books` 中间层。路由文档只管"这个关键词覆盖了哪些源库"，子库搜索靠 token 自己命中。
