...
索引总库 (yehuoshun/pbeshf)              ← 路由层：关键词→源库映射
    │
    ├── 📄 SpringBoot    → [{book_id, namespace, last_built}]
    ├── 📄 JVM           → [{book_id, namespace, last_built}]
    └── 📄 Canvas        → [{book_id, namespace, last_built}]
...
| 层 | 内容 | 作用 |
|----|------|------|
| 总库路由文档 | JSON `[{book_id, namespace, last_built}]` | 搜索路由（确认关键词已索引） + 构建追踪（源库映射、last_built 增量基准） |
...
标题：`{关键词}`（直接用关键词，无前缀符号。搜索时 token in:title 命中）

正文 — source_books 数组，记录该关键词下覆盖的源知识库：

```json
[
  {"book_id": 70910909, "namespace": "yehuoshun/dil9w3", "last_built": "2026-05-30T19:00:00Z"},
  {"book_id": 24256880, "namespace": "yehuoshun/abc123", "last_built": "2026-05-28T12:00:00Z"}
]
```
...
| `last_built` | 增量构建的时间基准。首次构建为空字符串，构建后更新为当前时间 |

> **关键词级路由**：一篇总库路由文档对应一个关键词。标题 = 关键词，body = source_books 数组。搜索时 token in:title 命中总库 → 确认关键词已索引 → 同 token in:title 搜子库命中索引文档 → 展开 entries。
>
...
6. 逐关键词 → yuque_create_doc → 总库创建路由文档（标题=关键词，body=source_books数组，last_built 为当前时间）
```
...
2-5. 同上（列文档 → 评分 → 聚合 → 写入）
6. 更新总库路由文档的 source_books（last_built 更新为当前时间）
```
...
用户提问: "springboot怎么连多个库"
         ...
         │     in:title 搜总库 → GET 路由文档 → 解析 source_books
