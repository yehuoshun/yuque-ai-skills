...
3. 逐文档评分（核心步骤，每文档只读一次 body）：
   ⚠️ **分批评分**：每批最多 **10 篇**。一次塞超过 10 篇会导致 LLM 输出量爆炸、自主偷懒砍别名、跳过 5 条规则展开。必须分批评分后汇总。
...
5. 逐关键词写入（子代理串行，并发=1，**原子操作**）：
   ...
   b. yuque_index_create(keyword, keywords[], summary, entries, index_book_id)
      ⚠️ 必须走 MCP 工具，禁止裸 API。MCP 工具内置 TOC 自动挂载 + 容量检查 + 去符号清洗。裸 API 绕过会漏 TOC。
      ⚠️ 写入完成后验证 TOC：curl /repos/{book_id}/toc 确认有子节点
...
字段说明：
- title：entry 文档标题
- keywords：该文档的技术术语 keywords 数组。⚠️ 必须包含该关键词的所有常见别名（如 CSharp 文档的 keywords 应含 C#、C井；JavaScript 的应含 JS），这样不同叫法搜到同一篇索引文档时都能命中。⚠️ **去符号版本必须保留原始版本作为 keywords 数组第二元素**（如 Threejs 的 keywords 应含 ["Threejs", "Three.js", ...]，CSharp 的 keywords 应含 ["CSharp", "C#", ...]），这样即使语雀将来支持符号搜索，原始版本也能命中
