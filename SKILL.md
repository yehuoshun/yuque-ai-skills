...
正文 — source_books 数组，记录该关键词索引所在的子索引库：

```json
[
  {"book_id": 79576301, "namespace": "yehuoshun/cgoza0"},
  {"book_id": 24256880, "namespace": "yehuoshun/abc123"}
]
```

| 字段 | 用途 |
|------|------|
| `book_id` | 子索引库 ID（关键词索引文档所在的子库） |
| `namespace` | 子索引库 namespace |

> **关键词级路由**：一篇总库路由文档对应一个关键词。标题 = 关键词，body = source_books 数组。source_books 指向子索引库——搜索时先搜总库确认关键词已索引 → 根据 source_books 确定去哪个子库 → 搜子库命中关键词索引文档 → 展开 entries。
>
> ⚠️ source_books 指向子索引库（如 cgoza0），不是源知识库。源知识库信息在子库索引文档的 entries 里。
...
6. 子代理完成 → 主会话读子索引库文档列表 → 逐关键词写入总库路由文档：
   ...
   source_books: [{"book_id": <子库id>, "namespace": "<子库ns>"}]
   > ⚠️ source_books 指向子索引库（如 cgoza0），不是源知识库。源库信息在子库索引文档 entries 里。
