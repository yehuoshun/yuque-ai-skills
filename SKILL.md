...
# 关键词索引架构已废弃，不再维护。基础 CRUD 工具仍可用。
# 替代方案：TOC 树导航（无向量 RAG）
---
name: yuque-ai
description: ⚠️ 已停止维护。语雀全功能技能。支持知识库管理、文档管理、小记管理、目录管理、文档导出、回收站管理。索引构建/知识库搜索/图谱已废弃，请使用 TOC 树导航替代。当用户提到「语雀」时触发。
---
...
```
yuque-mcp (MCP Server)     ← 管理操作：51 个 tools（CRUD、搜索、导出、导入、统计、群组、回收站、仪表盘、健康检查、配置管理、热重载、分组管理、索引构建与增量更新）
    ↓ 提供 51 个 tools
...
基于 MCP 51 tools 的高层业务能力。全部遵循：先预览后确认、单篇隔离不传染、上限 100 篇、结束出报告。
