...
>
> 分为三种场景：**全量构建-新建域** / **全量构建-复用子库** / **增量更新**。
...
4. 逐文档评分 → yuque_get_doc 读 body → LLM 输出 [{"keyword","w"}, ...]
5. 聚合去重 → 仅合并同义不同名，不合并语义不同概念
6. 逐关键词 → LLM 生成 keywords[]+summary → yuque_index_create 写入
7. yuque_update_repo → 更新 last_built
8. yuque_create_doc → 总库创建路由文档
```

#### B. 全量构建 — 复用已有子库

> 子库已在 `route_book_sub` 配好，跳过 1-2。

```
0. yuque_config_status → 前置检查（确认子库可用、容量 OK）
(跳过 1-2，直接用已有子库 book_id)
3. yuque_list_docs → 列出源库全部文档
4. 逐文档评分（同上）
5. 聚合去重（同上）
6. 逐关键词写入 → 已有关键词索引 update，新增 create
7. yuque_update_repo → 更新 last_built
8. 总库路由：先 yuque_search 搜是否已有该域路由文档
   ├─ 有 → yuque_update_doc 更新
   └─ 无 → yuque_create_doc 新建
```
...
3. 新增文档 → LLM 读正文 → 归入已有关键词或创建新关键词索引
...
7. yuque_update_repo → 更新子索引库 description 的 last_built
8. yuque_create_doc → 总库创建路由文档
```

### 1b. 复用已有子库

> 子库已在 `route_book_sub` 配好 → 跳过 1-2，从步骤 3 开始。步骤 8 需先搜后写。
...
6. 逐关键词写入：
   → 已存在该关键词索引？yuque_update_doc 更新 entries
   → 不存在？yuque_index_create 新建

7. yuque_update_repo → 更新 last_built

8. 总库路由文档登记：
   a. yuque_search(query="{domain}", scope=总库namespace) → 找已存在的路由文档
   b. 有 → yuque_update_doc 更新 body（追加新子库指针或更新 did）
   c. 无 → yuque_create_doc 新建路由文档
```
