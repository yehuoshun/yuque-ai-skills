...
```
yuque-mcp (MCP Server)     ← 管理操作：44 个 tools（CRUD、搜索、导出、导入、统计、群组、回收站、仪表盘、健康检查、配置管理、热重载）
    ↓ 提供 44 个 tools
...
基于 MCP 44 tools 的高层业务能力。全部遵循：先预览后确认、单篇隔离不传染、上限 100 篇、结束出报告。
...
```
索引总库 (yehuoshun/pbeshf)              ← 路由层：关键词→源库映射
    │
    ├── 📄 SpringBoot    → [{book_id, namespace}]
    ├── 📄 JVM           → [{book_id, namespace}]
    └── 📄 Canvas        → [{book_id, namespace}]

子索引库 (yehuoshun/cgoza0)               ← 数据层：关键词索引
    ├── 📄 SpringBoot    → search 面 + entries: [doc584, ...]
    ├── 📄 JVM           → search 面 + entries: [doc601, ...]
    └── 📄 Canvas        → search 面 + entries: [doc072, ...]
```
...
| 层 | 内容 | 作用 |
|----|------|------|
| 总库路由文档 | JSON `[{book_id, namespace}]` | 搜索路由（确认关键词已索引） |
| 子索引库描述 | 域名/简介 | 展示用途，不参与构建逻辑 |
...
标题：`{关键词}`（直接用关键词，无前缀符号。搜索时按标题精确匹配）

正文 — 子索引库文档指针数组，指向该关键词在子库中的具体索引文档：
...
| 字段 | 用途 |
|------|------|
| `did` | 子索引库中该关键词索引文档的 ID |
| `ns` | 子索引库中该索引文档的完整路径 `{book_ns}/{slug}` |

> **关键词级路由**：一篇总库路由文档对应一个关键词。标题 = 关键词，body 指向子索引库中的具体索引文档（did + ns）。搜索时先搜总库确认关键词已索引 → 通过 did/ns 直接 GET 索引文档 → 展开 entries。
>
> ⚠️ 指向子索引库的具体文档（did+ns），不是子库本身（book_id+ns）。

### 2.2 子索引库描述

子索引库的 `description` 字段仅存展示用途的元数据（如域名、简介），不参与构建逻辑。
...
[
  ...
  {"did":591,"ns":"yehuoshun/dil9w3","t":"条件装配与多数据源","s":"def","url":"https://www.yuque.com/yehuoshun/dil9w3/def","w":8}
]
```

> ⚠️ entries 必须放在 ` ```json ` 代码块内，避免语雀 Markdown 渲染器将 URL 误识别为链接导致格式错乱。`parseIndexDoc` 同时兼容新旧两种格式。
```
...
| entries | `entries：` + ` ```json ` 代码块 | 源文档指针列表 `[{did, ns, t, s, url, w}]`，**全部必填** |
...
>
> 两种构建模式：**全量构建**（新建域 / 复用子库）、**增量更新**（仅手动触发）。
...
1. yuque_create_repo → 创建子索引库（命名: {英文名}-{时间戳}，如 python-course-1714473600）
2. yuque_list_docs → 列出源库全部文档（标题 + id + slug + updated_at）
3. 逐文档评分 → yuque_get_doc 读 body → LLM 输出 [{"keyword","w"}, ...]
4. 聚合去重 → 仅合并同义不同名，不合并语义不同概念
5. 逐关键词 → LLM 生成 keywords[]+summary → yuque_index_create 写入
6. 逐关键词 → yuque_create_doc → 总库创建路由文档（标题=关键词，body=source_books数组）
```

#### B. 全量构建 — 复用已有子库

> 子库已在 `route_book_sub` 配好，跳过 1。

```
0. yuque_config_status → 前置检查（确认子库可用、容量 OK）
(跳过 1，直接用已有子库 book_id)
2-5. 同上（列文档 → 评分 → 聚合 → 写入）
6. 更新总库路由文档的 source_books
```

#### C. 增量更新

> 仅手动触发。子库已存在 + 已有索引，只更新变更的源文档。

```
1. 读总库路由文档 body → 拿到 source_books 数组
2. 遍历每个 source_book：
   a. yuque_list_docs(book_id) → 筛 updated_at > last_built
   b. 变更文档 → yuque_get_doc 读 body → LLM 评分 → [{keyword, w}, ...]
   c. 逐关键词写入子索引库：
      - 关键词索引已存在 → yuque_update_doc 追加/更新 entries 中该 did
      - 关键词索引不存在 → yuque_index_create 新建
3. 更新总库路由文档 source_books 中对应条目的 last_built
```

**增量边界处理**：
- **文档内容变了、旧关键词不再相关** → 旧 entries 保留为死链，搜索时 LLM 重排自动过滤，定期全量重建清理
- **文档被删了** → 不评分不更新，旧 entries 保留为死链，定期全量重建清理
- **文档完全没变** → `updated_at ≤ last_built`，跳过零成本

> **策略**：增量只管「新增和更新」，不管「清理」。清理交给定期全量重建。默认不开启自动增量，只提供手动触发。
...
用户提问: "springboot怎么连多个库"
         ...
         │
         ├─[1] LLM 生成 2-5 个独立搜索 token（每个 ≤1 个词，无空格无符号）
         │     "springboot怎么连多个库" → ["SpringBoot", "多数据源", "DataSource"]
         │
         ├─[2] Agent 调 yuque_kb_search(tokens)
         │     搜总库 → 客户端标题匹配过滤 → GET 路由文档 → 解析 source_books
         │     → 搜子库（route_book_sub）→ 客户端标题匹配过滤 → 命中关键词索引文档
...
> ⚠️ 步骤 0 通过后才能执行。全量构建适用于整个域首次建索引或需要完整重建的场景。
...
4. 聚合（LLM 判断）：所有文档的输出汇总
   → **归一化**：合并真正的同义不同名（如 "SpringBoot"="Spring Boot"="springboot"→ 统一为"SpringBoot"），LLM 判断
...
6. 子代理完成 → 主会话读子索引库文档列表 → 逐关键词写入总库路由文档：
   yuque_create_doc(总库, 标题=关键词) → body 写入子库索引文档指针
   body: [{"did": <子库索引文档id>, "ns": "<子库ns>/<子库文档slug>"}]
   > ⚠️ 指向子库的具体索引文档（did + ns完整路径），源库信息在子库索引文档 entries 里。
...
(跳过 1，直接用已有子库 book_id)
2-5. 同新建域（列文档 → 评分 → 聚合 → 写入子索引库）

[主会话 — 阶段 2：总库路由同步]

6. 子代理完成 → 主会话读子索引库文档列表 → 逐关键词同步总库路由：
   逐关键词 → 检查总库是否已有该关键词路由文档
   - 已有 → yuque_update_doc 更新 body（合并/去重 did，追加新索引文档指针）
   - 没有 → yuque_create_doc(总库, 标题=关键词, body=子库索引文档指针数组)
   > ⚠️ body 用 [{did, ns}] 格式，指向子库的具体索引文档。
   > ⚠️ 此阶段回主会话执行，不放在子代理里。
```

## 2. 增量更新（仅手动触发）

> ⚠️ 默认不开自动增量。用户主动触发时才执行。
> 增量只负责「新增和更新」，不管清理（死链靠定期全量重建）。

```
1. 收集所有涉及的关键词 → 逐关键词读总库路由文档 body → 拿到 source_books 数组
   (合并去重所有 source_book，得到唯一的源库列表)

2. 遍历每个 source_book：
   a. yuque_list_docs(book_id) → 筛 updated_at > last_built → 变更文档列表
   b. 对每个变更文档：
      - yuque_get_doc 读 body → LLM 评分 → [{keyword, w}, ...]
      - 逐关键词写入子索引库：
        · 关键词索引已存在 → yuque_update_doc 更新 entries（覆盖该 did 的旧条目）
        · 关键词索引不存在 → yuque_index_create 新建
   c. 逐关键词更新总库路由文档：该关键词路由文档的 source_books 中对应 book_id 的 last_built
```

**边界处理**：
- **文档内容变了、旧关键词不再相关** → 新评分覆盖，旧关键词索引中的 entries 变成死链。搜索时 LLM 重排自动过滤，定期全量重建清理
- **文档被删了** → 列表不出现，不评分不更新，旧 entries 留为死链
- **文档完全没变** → `updated_at ≤ last_built`，跳过

| 模式 | 触发方式 | 默认 | 适用场景 |
|------|---------|------|---------|
| 全量重建 | 手动 | ✅ | 首次建索引、死链积累多、大改源文档 |
| 手动增量 | 用户主动触发 | ✅ | 刚写完几篇，立刻想搜到 |
| 自动增量 | 定时任务 | ❌ 默认关闭 | 不建议，死链积累不可控 |
...
> 3. 用户确认 → `yuque_create_repo("{英文名}-{时间戳}")`
